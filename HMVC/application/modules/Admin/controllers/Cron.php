...
class Cron extends CI_Controller
{
    ...
    public function roiCron()
    {
        ...
        if (empty($cron)) {
            ...
            if ($setting['roi_income'] == 0) {
                $roi_users = $this->Main_model->get_records('tbl_roi', array('days >' => 0, 'status' => 0), '*');
                ...
                foreach ($roi_users as $key => $user) {
                    ...
                    if ($diff >= 0) {
                        ...
                        if ($userinfo['roi_limit'] > $userinfo['roi_pending']) {
                            $totalCredit = $userinfo['roi_pending'] + ($roi_amount);
                            ...
                            pr($incomeArr);
                            $this->Main_model->add('tbl_income_wallet', $incomeArr);
                            $this->Main_model->update('tbl_users', ['user_id' => $user['user_id']], ['roi_pending' => ($userinfo['roi_pending'] + $incomeArr['amount'])]);
                            ...
                            //  $level_income = '0.2,0.15,0.1,0.1,0.05,0.05,0.05,0.05,0.05,0.05,0.03,0.03,0.03,0.03,0.03,0.01,0.01,0.01,0.01,0.01';
    ...
    private function level_income($sponser_id, $activated_id, $package_income, $package)
    {
        ...
        foreach ($incomeArr as $key => $income) {
            ...
            // $direct = $key + 1;
            $setting = get_single_record('tbl_settings', array('id' => 1), '*');
            if ($setting['level_income'] == 0) {
                $sponser = get_single_record('tbl_users', array('user_id' => $sponser_id), 'id,user_id,sponser_id,paid_status,package_amount,directs,total_limit,pending_limit');
                if (!empty($sponser)) {
    ...
    //         $userinfo = $this->Main_model->get_single_record('tbl_users', ['user_id' => $user_id], 'user_id,sponser_id,directs,incomeLimit,incomeLimit2,total_package');
    ...
    public function BoosterAchiever()
    {
        ...
        $cron = $this->Main_model->get_single_record('tbl_cron', ['date' => $date, 'cron_name' => 'booster'], '*');
        if (empty($cron)) {
            $this->Main_model->add('tbl_cron', ['cron_name' => 'booster', 'date' => $date]);
    ...
    public function Auto_block_user()
    {
        $unpaidusers = get_records('tbl_users', ['paid_status' => 0], '*');
        foreach ($unpaidusers as $key => $user) {
            $date1 = date('Y-m-d H:i:s');
            $date2 = date('Y-m-d H:i:s', strtotime($user['created_at'] . '+ 15 days'));
            $diff = strtotime($date1) - strtotime($date2);
            if ($diff > 0) {
                $userdata = [
                    'user_id' => $user['user_id'],
                    'type' => 'Auto Block User Id',
                    'disabled' => 1,
                ];
                add('tbl_autoblock_users', $userdata);
                update('tbl_users', ['user_id' => $user['user_id']], ['disabled' => 1]);
            }
        }
    }
    public function activateTrade()
    {
        $users = $this->Main_model->activatetrade();
        foreach ($users as $user) {
            $trade = get_single_record('trading_wallet', ['user_id' => $user['user_id'], 'type' => 'account_activation'], 'user_id,ifnull(sum(amount),0) as total');

            if (!empty($trade['user_id']) && $user['package'] > $trade['total']) {
                pr($user);
                pr($trade);

                $total_amount = ($user['package'] - $trade['total']);
                $addWallet = array(
                    'user_id' => $user['user_id'],
                    'amount' => $total_amount,
                    'type' => 'account_activations',
                    'remark' => 'Account Activation Amount for ' . $user['user_id'],
                );
                add('trading_wallet', $addWallet);
            }
        }
    }
}
