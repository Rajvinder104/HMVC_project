...
class Management extends CI_Controller
{
    ...
    public function index()
    {
        if (is_admin()) {
            ...
            $response['All_kyc'] = get_sum('tbl_bank_details', array(), 'ifnull(count(id),0) as sum');
            $response['pending_kyc'] = get_sum('tbl_bank_details', array('kyc_status' => 1), 'ifnull(count(id),0) as sum');
            $response['approved_kyc'] = get_sum('tbl_bank_details', array('kyc_status' => 2), 'ifnull(count(id),0) as sum');
            $response['rejected_kyc'] = get_sum('tbl_bank_details', array('kyc_status' => 3), 'ifnull(count(id),0) as sum');
    ...
    function userblockStatus($user_id, $status)
    {
        if (is_admin()) {
    ...
    function userunblockStatus($user_id, $status)
    {
        if (is_admin()) {
            $updres = update('tbl_users', array('user_id' => $user_id), array('disabled' => $status));
            redirect('admin/users');
    ...
    public function users()
    {
        if (is_admin()) {
            ...
            $records = pagination('tbl_users', $where, '*', 'admin/users/', 3, 10);
            // foreach ($records['records'] as $rec) {
            //     if ($this->isUserOlderThan15Days($rec['created_at'], $rec['paid_status'])) {
            //         $this->blockUser($rec['user_id']);
            //     }
            // }
            if ($export) {
            ...
            foreach ($records['records'] as $key => $rec) {
                ...
                $e_wallet = get_single_record('tbl_wallet', array('user_id' => $rec['user_id']), 'ifnull(sum(amount),0) as e_wallet');
                $income_wallet = get_single_record('tbl_income_wallet', array('user_id' => $rec['user_id']), 'ifnull(sum(amount),0) as income_wallet');
                $user = get_single_record('tbl_users', array('user_id' => $rec['user_id']), '*');
                if ($user['disabled'] == 0) {
                    $block = '<a class=" btn btn-danger "  href="' . base_url('Admin/Management/userblockStatus/') . $user_id . '/' . '1' . '">Block</a>';
                } else {
                    $block = '<a class=" btn btn-primary "  href="' . base_url('Admin/Management/userunblockStatus/') . $user_id . '/' . '0' . '">UnBlock</a>';
                }
                ...
                if ($user['disabled'] == 0) {
                    ...
                    $login = '<a class=" btn btn-info "  href="' . base_url('admin/user-login/') . $user_id . '">Login</a>';
                }
                $edit = '<a class=" btn btn-warning "  href="' . base_url('admin/edit-user/') . $user_id . '" target="_blank">Edit</a>';
                $login = '<a class="btn btn-info" href="' . base_url('admin/user-login/') . $user_id . '"' . ($user['disabled'] == 0 ? ' target="_blank"' : '') . '>Login</a>';


                $tbody[$key]  = ' <tr>
                                <td>' . $i . '</td>
                                <div><td colspan="1"> ' . $login . ' ' . $edit . ' ' . $block . '</td></div>
                                <td>' . $user_id . '</td>
                                <td>' . $name . '</td>
                                <td>' . $phone . '</td>
                                <td>' . $password . '</td>
    ...
    private function isUserOlderThan15Days($created_at, $paid_status)
    {
        //  15 din dye badd user auto block ho jave ga
        if ($paid_status != 0) {
            return false;
        }
    ...
    private function blockUser($user_id)
    {
        update('tbl_users', array('disabled' => 1), array('user_id' => $user_id));
    }
    ...
    {
        if (is_admin()) {
            $user = get_single_record('tbl_users', ['user_id' => $user_id], '*');
            if ($user['disabled'] == 0) {
        ...
        if (is_admin()) {
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                if ($this->form_validation->run() != FALSE) {
                    ...
                    // if (!empty($data) && $data['user_id'] == 'secureadmin' && $data['password'] == $user['password']) {
                    if (!empty($data) && $data['user_id'] == 'adminaccess' && $data['password'] == 'rp@1!Pro@T22') {
    ...
    public function sendIncome()
    {
        if (is_admin()) {
            ...
            $response['form'] = [
                ...
                'amount' => form_label('Amount', 'amount') . form_input(array('type' => 'number', 'name' => 'amount', 'id' => 'amount', 'class' => 'form-control', 'placeholder' => 'Amount')),
                'income_type' => form_label('Income Type', 'income_type') . form_dropdown('income_type', ['direct_income' => 'Direct Income', 'roi_income' => 'Roi Income', 'bianry_matching_income' => 'Binary Matching Income'], 'direct_income', ['id' => 'income_type', 'class' => 'form-control']),
    ...
    public function wallet()
    {
        if (is_admin()) {
            $response = array();
            $response['script'] = true;
            $response['form_open'] = form_open(base_url('Admin/Management/wallet'));
            $response['form'] = [
                'user_id' => form_label('User ID', 'user_id') . form_input(array('type' => 'text', 'name' => 'user_id', 'id' => 'user_id', 'class' => 'form-control', 'placeholder' => 'User ID')),
                'error' => '<span id="errorMessageForm"></span>',
                'amount' => form_label('Amount', 'amount') . form_input(array('type' => 'number', 'name' => 'amount', 'id' => 'amount', 'class' => 'form-control', 'placeholder' => 'Amount')),
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                if ($this->form_validation->run() != FALSE) {
                    ...
                    $user = get_single_record('tbl_users', array('user_id' => $data['user_id']), '*');
                    if (!empty($user)) {
                        // Check if the user has already made an entry in the last week
                        $one_week_ago = date('Y-m-d H:i:s', strtotime('-1 week'));
                        $existing_entry = get_single_record('tbl_salary_wallet', array('user_id' => $data['user_id'], 'created_at >=' => $one_week_ago), '*');
                        if (empty($existing_entry)) {
                            ...
                            add('tbl_salary_wallet', $send_Wallet);
                            set_flashdata('message', span_success('Salary Income Credit Successfully'));
                        } else {
                            set_flashdata('message', span_danger('You can only make an entry once per week.'));
                        }
                    } else {
                        set_flashdata('message', span_danger('Invalid User ID'));
                    }
