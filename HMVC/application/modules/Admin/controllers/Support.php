...
class Support extends CI_Controller
{
    ...
    public function inbx()
    {
        if (is_admin()) {
            $where = ['status' => 0];
            $response['users'] = get_records('tbl_support_message',  $where, '*');
            $this->load->view('inbox', $response);
        }
    }
    public function outbx()
    {
        if (is_admin()) {
            $where = [];
            $response['users'] = get_records('tbl_support_message', $where, '*');
        ...
        if (is_admin()) {
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                if ($this->form_validation->run()) {
                  ...
                  $userdata =  $this->Main_model->update('tbl_support_message', ['id' => $id],  $userdata );
                  if($userdata){
                    $this->session->set_flashdata('message', span_success('Data updated Successfully'));
                  }else{
                    $this->session->set_flashdata('message', span_danger('Data Not Updated Please Try Again'));
                  }
                }
            }
            $response['user'] = get_single_record('tbl_support_message', ['id' => $id], '*');
            $this->load->view('approvel', $response);
        }
    }
}
