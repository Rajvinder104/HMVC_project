...
class User_model extends CI_Model
{
    ...
    {
        $this->db->select('ifnull(count(tbl_sponser_count.downline_id),0) as team, tbl_users.user_id');
        $this->db->from('tbl_users');
    ...
    public function TodaygetBusiness($user_id)
    {
        $today = date('Y-m-d');
        $this->db->select('IFNULL(SUM(tbl_users.total_package), 0) as teamBusiness');
        $this->db->from('tbl_sponser_count');
        ...
        $this->db->where('DATE(tbl_users.created_at) =', $today);
        $query = $this->db->get();
        $res = $query->row_array();
        return $res;
    }
    ...
    public function totalteam($user_id, $status)
    {
    ...
    public function Allteam($user_id)
    {
        $this->db->select('ifnull(count(tbl_sponser_count.downline_id),0) as team ,tbl_users.user_id');
        $this->db->from('tbl_users');
        $this->db->join('tbl_sponser_count', 'tbl_users.user_id = tbl_sponser_count.downline_id');
    ...
    public function grouped_by_levels($where, $limit, $offset)
    {
        $this->db->select('level');
        $this->db->group_by('level');
        $this->db->limit($limit, $offset);
        $query = $this->db->get_where('tbl_sponser_count', $where);
        $res = $query->result_array();
        return $res;
    }
    ...
    public function getJoinedData()
    {
        $this->db->select('*');
        $this->db->from('tbl_sponser_count');
        $this->db->join('tbl_users', 'tbl_sponser_count.user_id = tbl_users.user_id', 'inner');
        $query = $this->db->get();
        return $query->result();
    }
    ...
    //     return $res;
    // }
    public function groupslevel($where, $limit, $offset)
    {
        $this->db->select('level');
        $this->db->group_by('level');
        $this->db->limit($limit, $offset);
    ...
    public function getrecords($table, $where, $select, $limit, $offset)
    {
        $this->db->select($select);
        $this->db->where($where);
        $this->db->limit($limit, $offset);
...
// -----------------------------------------------------
    public function payout_sum_by_week($table, $where)
    {
        $this->db->select('YEARWEEK(created_at, 1) as week, SUM(amount) as total_payout');
        $this->db->from($table);
        $this->db->where($where);
        $this->db->group_by('YEARWEEK(created_at, 1)');
        $query = $this->db->get();
        return $query->result_array();
    }

    public function week_wise_payout_summary($where, $limit, $offset)
    {
        $this->db->select('YEARWEEK(created_at, 1) as week, MIN(created_at) as week_start_date, MAX(created_at) as week_end_date');
