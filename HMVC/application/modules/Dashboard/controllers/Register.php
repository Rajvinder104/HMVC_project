...
class Register extends CI_Controller
{
    ...
    public function simpleRegister()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($sponser_id === 'admin') {
                    set_flashdata('register_message', span_danger('Admin cannot be Sponser'));
                    $this->load->view('register', $response);
                } else {
                    if (!empty($sponser)) {
                        // if(empty($checkEmail)){
                        ...
                        $userData['name'] = $this->input->post('name');
                        $userData['phone'] = $this->input->post('phone');
                        $userData['password'] =  $this->generatePassword(); //$this->input->post('password');
    ...
    function generatePassword($length = 10)
    {
        ...
        $allChars = $upper . $lower . $numbers . $specialChars;

        // Ensure the password is long enough and include at least one of each type
        $password = '';
    ...
    public function testmail2()
    {
        ...
        $userData['email'] = 'gnirajvinder@gmail.com';
        $userData['master_key'] = '1234';
        $sms_text = '<div style=\'background-color:#222f3d; border:1px #d9cece solid;padding:30px;border-radius:10px;margin: 30px;\'><div style=\'text-align:start;margin-bottom:30px;\'><img src=\'https://plexproglobal.com/assets/images/logo/logo.png\'  style=\'max-width: 160px;\'><div><h4 style=\'color:#fff; \'></h4></div> </div><div><p style=\'color:#fff;\'>Welcome to ' . 'Name : ' . '(' . $userData['name'] . ')' . ' User ID : ' . '(' . $userData['user_id'] . ') ' . 'Password :' . '(' . $userData['password'] . ')' . 'Transaction Password :' .  '(' . $userData['master_key'] . ')' . '  ! We are delighted to have you join our community.</p><p style=\'color:#fff;\'>To activate your account and gain access to all the features, please follow these simple steps:</p><p style=\'color:#fff;\'</p><p style=\'color:#fff;\'>If you have any questions or encounter any issues during the activation process, feel free to reach out to our support team.</p><p style=\'color:#fff;\'>Thank you for choosing plexproglobal. We look forward to serving you!</p></div></div></div>';
        composeMail($userData['email'], 'Registration', $sms_text);
    }
}
