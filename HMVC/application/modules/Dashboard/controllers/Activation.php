...
class Activation extends CI_Controller
{
    ...
    public function reinvest()
    {
        if (is_logged_in()) {
            ...
            $response['tokenValue'] = get_single_record('tbl_token_value', ['id' => 1], 'amount');
            $response['wallet'] = get_single_record('tbl_wallet', array('user_id' => $this->session->userdata['user_id']), 'ifnull(sum(amount),0) as wallet_balance');
            $response['packages'] = get_records('tbl_time_update', [], '*');
            $this->load->view('reinvestMent', $response);
        } else {
            redirect('login');
        }
    }
        ...
        if (is_logged_in()) {
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                if ($this->form_validation->run() !== FALSE) {
                    ...
                    $wallet = get_single_record('tbl_wallet', array('user_id' => $this->session->userdata['user_id']), 'IFNULL(SUM(amount), 0) as wallet_balance');
                    $date = get_single_record('tbl_activation_details', array('user_id' => $this->session->userdata['user_id']), '*');
                    $package = get_single_record('tbl_time_update', ['id' => $data['package_id']], '*');

                    $activationAmount = $package['price'];
                    $days = $this->getPackageDays($activationAmount);

                    if ($wallet['wallet_balance'] >= $activationAmount) {
                        if (!empty($user)) {
                            $existingReinvestment = get_single_record('tbl_reinvestment_details', ['user_id' => $user_id, 'package' => $activationAmount, 'type' => 'Reinvestment'], '*');

                            if ($existingReinvestment) {
                                $response['message'] = span_danger_simple('Account Already Reinvested');
                            } else {
                                if ($user['paid_status'] >= 0) {
                                    ...
                                    $res = add('tbl_reinvestment_details', $activationData);
                                    // $res = update('tbl_reinvestment_details', array('user_id' => $user_id) , $activationData);

                                    if ($res) {
                                        $sendWallet = array(
                                            'user_id' => $this->session->userdata['user_id'],
                                            'amount' => -$activationAmount,
                                            'type' => 'account_reinvestment',
                                            'remark' => 'Account Reinvestment Deduction for ' . $user_id,
                                        );

                                        add('tbl_wallet', $sendWallet);

                                        $response['status'] = true;
                                        $response['message'] = span_success_simple('Account Reinvested Successfully');
                                    } else {
                                        $response['message'] = span_info_simple('Error occurred while processing reinvestment.');
                                    }
                                } else {
                                    $response['message'] = span_info_simple('Please activate this account first.');
                                }
                            }
                        } else {
                        ...
                        $response['message'] = span_danger_simple('Insufficient Balance!');
                    }
                } else {
                    $response['message'] = span_danger_simple(validation_errors());
                }
                redirect('dashboard/reinvest', $response);
    ...
    private function getPackageDays($amount)
    {
        $daysMapping = [
            5 => 5,
            15 => 10,
        ...
        return isset($daysMapping[$amount]) ? $daysMapping[$amount] : 0;
    }
        ...
        if (is_logged_in()) {
            ...
            if (activation_process == 0) {
                ...
                if ($this->input->server('REQUEST_METHOD') == 'POST') {
                                ...
                                if ($user['paid_status'] == 0) {
                                    if ($activationAmount >= 1000) {
                                        if ($activationAmount % 1000 == 0) {
                                            ...
                                            if ($user['paid_status'] == 0) {
                                                $topupData['topup_date'] = date('Y-m-d H:i:s');
                                            }
                                            if ($data['package_option'] == 'with_poduct') {
                                                $topupData['roi_limit'] = $user['roi_limit'] + ($activationAmount * 1);
                                            } else {
                                                $topupData['roi_limit'] = $user['roi_limit'] + ($activationAmount * 2);
                                            }
                                            if (income_limit == 0) {
                                                $topupData['total_limit'] = $user['total_limit'] + ($activationAmount * 3);
                                            }
                                            ...
                                            if ($res) {
                                                ...
                                                if (roi_access == 0) : // ROI START IF
                                                    if ($activationAmount >= 1000 && $activationAmount <= 10000) {
                                                        $per = 0.50;
                                                    } elseif ($activationAmount >= 11000 && $activationAmount <= 50000) {
                                                        $per = 0.60;
                                                    } elseif ($activationAmount >= 51000 && $activationAmount <= 100000) {
                                                        $per = 0.75;
                                                    } elseif ($activationAmount >= 101000 && $activationAmount <= 200000) {
                                                        $per = 1;
                                                    } elseif ($activationAmount >= 201000) {
                                                        $per = 1.5;
                                                    }

                                                    $roiArr = array(
                                                        'user_id' => $user['user_id'],
                                                        'amount' => $activationAmount * $per * 200,
                                                        'roi_amount' => ($activationAmount * $per / 100),
                                                        'days' => 200,
                                                        'total_days' => 200,
                                                        'package' => $activationAmount,
                                                        'type' => 'roi_income',
                                                ...
                                                $setting = get_single_record('tbl_settings', array('id' => 1), '*');
                                                if ($setting['direct_income'] == 0) {
                                                    ...
                                                    if (!empty($sponser['user_id']) && $sponser['paid_status'] == 1) {
                                                        if (direct_access == 0) : // DIRECT START IF
                                                            ...
                                                            if ($sponser['total_limit'] > $sponser['pending_limit']) {
                                                                $totalCredit = $sponser['roi_pending'] + ($activationAmount * 0.05);
                                            ...
                                            $response['message'] = span_info_simple('Activation Amount is Multiple of  ' . currency . ' 1000');
                                        }
                                    } else {
                                        $response['message'] = span_info_simple('Minimum Activation Amount is ' . currency . ' 1000');
                                    }
                                } else {
                    ...
                    redirect('dashboard/activate-account');
                    // echo json_encode($response);
                    // exit;
                }
            } else {
    ...
    private function boosterAchiver($user_id)
    {
        ...
        $date2 = date("Y-m-d H:i:s", strtotime('+7 days', strtotime($sponser['topup_date']))); //  + 10 days
    ...
    public function myactivateview()
    {
        if (is_logged_in()) {
            $response['wallet'] = get_single_record('tbl_wallet', array('user_id' => $this->session->userdata['user_id']), 'ifnull(sum(amount),0) as wallet_balance');
            if (activation_process == 0) {
                                ...
                                if ($user['paid_status'] == 0) {
                                    if ($activationAmount >= 1000) {
                                        if ($activationAmount % 1000 == 0) {
                                            ...
                                            set_flashdata('message', span_danger('Activation Amount is Multiple of  ' . currency . ' 1000'));
                                        }
                                    } else {
                                        set_flashdata('message', span_danger('Minimum Activation Amount is ' . currency . ' 1000'));
                                    }
                                } else {
                                }
                            } else {
            ...
            $this->load->view('myactivation', $response);
        } else {
            redirect('login');
        }
    }
    ...
    private function level_income2($sponser_id, $activated_id, $package_income, $package)
    {
        $incomeArr = explode(',', $package_income);
