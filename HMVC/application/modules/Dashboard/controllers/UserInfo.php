...
class UserInfo extends CI_Controller
{
    ...
    {
        if (is_logged_in()) {
           // $response['notification'] = get_sum('tbl_support_message', ['user_id' => $this->session->userdata['user_id'],'status' => 1], 'ifnull(count(id),0) as sum');
            $response['totalsum'] = get_sum('tbl_wallet', ['user_id' => $this->session->userdata['user_id']], 'ifnull(sum(amount),0) as sum');
            $response['usageamount'] = get_sum('tbl_wallet', ['user_id' => $this->session->userdata['user_id'], 'amount <' => 0], 'ifnull(sum(amount),0) as sum');
            $response['allamount'] = get_sum('tbl_wallet', ['user_id' => $this->session->userdata['user_id'], 'amount >' => 0], 'ifnull(sum(amount),0) as sum');
            ...
            $response['Paidteam'] = $this->User_model->totalteam($this->session->userdata['user_id'], 1);
            $response['UnPaidteam'] = $this->User_model->totalteam($this->session->userdata['user_id'], 0);
            $response['Totalteam'] = $this->User_model->Allteam($this->session->userdata['user_id']);
    ...
    public function get_user2($user_id = '')
    {
        $response = array();
        $response['success'] = 0;
        $user = get_single_record('tbl_users', array('user_id' => $user_id), 'id,user_id,sponser_id,role,name,email,phone,paid_status,created_at');
