    ...
    public function index()
    {
        if (is_logged_in()) {
            ...
            $response['form'] = [
                ...
                'father_name' => form_label('Father Name', 'father_name') . form_input(array('type' => 'text', 'name' => 'father_name', 'id' => 'father_name', 'class' => 'form-control', 'value' => '' . $this->userinfo->father_name . '', 'placeholder' => 'Enter Your Father Name')),
                'pan_card' => form_label('Pan Card', 'pan_card') . form_input(array('type' => 'text', 'name' => 'pan_card', 'id' => 'pan_card', 'class' => 'form-control', 'value' => '' . $this->userinfo->pan . '', 'placeholder' => 'Enter Pan Card')),
                'aadhar' => form_label('Aadhar No', 'aadhar') . form_input(array('type' => 'text', 'name' => 'aadhar', 'id' => 'aadhar', 'class' => 'form-control', 'value' => '' . $this->userinfo->aadhar . '', 'placeholder' => 'Enter Aadhar Card')),
                ...
                'dob' => form_label('DOB', 'dob') . form_input(array('type' => 'date', 'name' => 'dob', 'id' => 'dob', 'class' => 'form-control', 'value' => '' . $this->userinfo->dob . '', 'placeholder' => 'Enter dob')),
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                $this->form_validation->set_rules('pan_card', 'Pan Card', 'trim|required|xss_clean');
                $this->form_validation->set_rules('aadhar', 'Aadhar No', 'trim|required|numeric|min_length[12]|max_length[12]|xss_clean');
                ...
                if ($this->form_validation->run() != FALSE) {
                    if ($this->userinfo->master_key == $data['txn_password']) {
                        ...
                        $Userdata['father_name'] = $data['father_name'];
                        $Userdata['pan'] = $data['pan_card'];
    ...
    public function zilUpdate()
    {
        if (is_logged_in()) {
            ...
            $response['form'] = [
                ...
                'otp' => form_label('OTP', 'otp') . form_input(array('type' => 'text', 'name' => 'otp', 'id' => 'otp_input', 'class' => 'form-control', 'style' => 'display: none;',  'placeholder' => 'Enter OTP')),
            ];
            $response['form_button'] = [
                'otp' =>  form_button('otp', 'GET OTP', ['class' => 'btn btn-info', 'id' => 'otp']),
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                if ($this->form_validation->run() != FALSE) {
                    ...
                    if($data['otp'] == $_SESSION['verification_otp']){
                        $this->session->unset_userdata('verification_otp');
        ...
        if (is_logged_in()) {
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    if ($updres) {
                        set_flashdata('message', span_success('Account Details Updated Successfully'));
                        // redirect('dashboard/uploaddocument');
                    } else {
                        set_flashdata('message', span_danger('Verify There is an error while Updating Account Details Please Try Again'));
                    }
                } else {
            ...
            $response['user'] = get_single_record('tbl_bank_details', ['user_id' => $this->session->userdata['user_id']], '*');
