    ...
    public function Sendmail()
    {
        if (is_logged_in()) {
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                $this->form_validation->set_rules('message', 'Description', 'required|trim');
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    $updres = add('tbl_support_message', $senddata);
                }
                if ($updres == true) {
                    set_flashdata('messages', span_success('Mail Composed Successfully'));
                } else {
                    set_flashdata('messages', span_danger('There is an error while updating Bank details Please try Again ..'));
                }
            }
            $this->load->view('composemail');
        }
    }
    public function inboxreport(){
      
        $where = ['user_id' => $this->session->userdata['user_id'],'status' => 1];
        $response['users'] = $this->User_model->get_records('tbl_support_message', $where,'*');
        $update_data = array(
            'notification' => 2 
        );
        $this->User_model->update('tbl_support_message', $where, $update_data);
        $this->load->view('mailbox',$response);
    }
    public function outboxreport(){
