        ...
        <div class="">
            ...
            <div class="row">
                ...
                <?php echo $this->session->flashdata('withdraw_bank_message'); ?>
                    ...
                    if($userinfo->withdraw_option =="daily"){
                        if(date('D') != "Sun" &&  date('D') == "Fri"){
                            $withdrawopen = true;

                        }
                    }elseif($userinfo->withdraw_option =="weekly"){
                        // $days = 7;
                        if(date('D') == "Fri"){
                            $withdrawopen = true;
                        }
                    }elseif($userinfo->withdraw_option =="mothly"){
                        if(date('d') == 2){
                            $withdrawopen = true;
                        }
                       
                    }else{
                        if($userinfo->withdraw_option ==""){
                            $withdrawopen = true;
                        }
                    }

                    
                    if ($withdrawopen == true) {
                    ?>
                    <select  name="option" onchange ="chocieOption(this)">
                            ...
                            <div id="rootwizard" class="wizard-full-width border-0">
                                <div class="wizard-content tab-content p-0">
                                    ...
                                    <div class="tab-pane active show" id="tabFundRequestForm">
                                        <!-- BEGIN row -->
                                        <div class="row" id="OffWithdraw" style ="display:none">
                                            <!-- BEGIN col-6 -->
                                            <?php if (withdraw_status == 0) { ?>
                                                <div class="col-md-12">
                                                    ...
                                                    <?php
                                                    // if (date('D') == 'Wed') {
                                                    if (date('H:i') >= '06:00' && date('H:i') <= '17:00') {
                                                    ...
                                                    ?>
                                                        ...
                                                        <div class="form-group">
                                                            <label>Amount</label>
                                                            <input type="hidden" class="form-control" name="withdraw_option"  id="optionWithdraw" />
                                                ...
                                                <div class="form-group">
                                                <?php echo ' <button type="subimt" name="save" class="btn btn-info" />Withdrawal Now</button>' ;?>

                                                    <!-- <?php //echo ($bankinfo->kyc_status == 2 ? ' <button type="subimt" name="save" class="btn btn-info" />Withdrawal Now</button>' : ($bankinfo->kyc_status == 1 ? badge_info('Wait For Kyc Approval') : '<a href="' . base_url('dashboard/account-details') . '" class="btn btn-danger" />Submit KYC First</a>')); ?> -->
                                                </div>

                                        <?php
                                                        echo form_close();
                                                    } else {
                                                        echo '<span class="text-danger">Withdraw Only 06AM To 5PM !</span>';
                                                    }
                                                } else {
...
function chocieOption(e){
     var option = e.value;
     if(option =="select"){
        $("#OffWithdraw").css("display", "none");
     }else{
        $("#OffWithdraw").css("display", "block");
        document.getElementById('optionWithdraw').value = e.value;
     }
